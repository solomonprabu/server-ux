...
patch(KanbanRecord.prototype, {
    createWidget(props) {
        ...
        this.dataState.widget.hasArchiveAccess = this.env.archivePermissions ? this.env.archivePermissions.archive : true;
        this.dataState.widget.hasUnarchiveAccess = this.env.archivePermissions ? this.env.archivePermissions.unarchive : true;
    }
});
...
patch(KanbanHeader.prototype, {
    canArchiveGroup() {
        ...
        return canArchive && (this.env.archivePermissions ? this.env.archivePermissions.archive : true);
    }
});
...
patch(KanbanCompiler.prototype, {
    compileButton(el, params) {
        ...
        if (type === "archive") {
            ...
            el.setAttribute("t-if", existingIf ? `(${existingIf}) and widget.hasArchiveAccess` : "widget.hasArchiveAccess");
        }
        if (type === "unarchive") {
            const existingIf = el.getAttribute("t-if");
            el.setAttribute("t-if", existingIf ? `(${existingIf}) and widget.hasUnarchiveAccess` : "widget.hasUnarchiveAccess");
        }
